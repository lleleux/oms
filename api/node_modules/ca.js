		...
		if (ca) {
			...
			dbCommander.command('init-agent', parameters, function(result) {
				if (result.code == 0) {
					...
					var cert = split[1];
					installsDao.update(agentId, {private_key: key, certificate: cert}, function (result) {
						if (!result || result !== 1) {
			...
			configDao.findOne({name: 'certificationAuthority'}, function (ca) {
				if (ca) {
					...
					dbCommander.command('generate-crl', parameters, function (result) {
						configDao.update(ca._id, {'crl.contents' : result.data, 'crl.last_generation' : new Date().getTime()}, function (result) {
							if (!result || result !== 1) {
