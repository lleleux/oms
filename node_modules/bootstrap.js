...
var bootstrap = function (service, serviceGracefulExitCallback, callback) {
	...
	db.open(bootstrapConfig.dbHost, bootstrapConfig.dbPort, ['oms', 'doc'], function () {
		// Load configuration
		config.load(os.hostname(), service, function (err) {
			// If error, throw it...
			if (err) {
				logger.error('[Bootstrap] Unable to load configuration: ' + err.message);
				throw err;
			}
			// Register the service
			registration.register(function () {
				// Bootstrap the service
				_bootstrapService(service, callback);
			});
		});
	});
};
...
var _bootstrapService = function (service, callback) {
