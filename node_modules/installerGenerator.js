...
var generateServices = function (callback) {
	...
	_getSources(function (err) {
		...
		configDao.findOne({name: 'installers'}, function (err, installers) {
...
var generateAgent = function(agentId, callback) {
	...
	_getSources(function (err) {
		...
		configDao.findOne({name: 'installers'}, function (err, installers) {
			...
			installDao.findById(agentId, function (err, agent) {
				if (agent) {
					// Get/Set infos
					var infos = installers.debian['oms-agent'];
					infos.key = agent.private_key;
					infos.certificate = agent.certificate;
					infos.sourcesDir = sourcesPath;
					infos.installersDir = installPath;
					infos.agentId = agentId;
					// Generate
					dbCommander.command('create-agent-installer', infos, function (result) {
						if (result.code === 0) {
							logger.info('[Installer Generator] "' + infos.name + '" installer generated for agent ' + agentId);
							if (callback) callback(null);
						} else {
							if (callback) callback(new Error('[Installer Generator] Unable to generate installer for agent ' + agentId + ': ' + err.message));
						}
					});
				} else {
...
/**
 * Generate an service installer with the given infos.
 ...
 * } ]
 *
 * @return an array of all the available installers
 */
