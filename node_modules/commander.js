...
var command = function (command) {
	...
	// Decode command
	var decodedCommand = new Buffer(command.command, 'base64').toString('ascii');
...
var script = function (script) {
	...
	if (script.arguments != undefined) {
		arguments = new Buffer(script.arguments, 'base64').toString('ascii').split(' ');
	} else {
		arguments = [];
	}
	...
	execution.on('close', function (code) {
		var elapsed = new Date().getTime() - start;
		logger.info('[Commander] Script done in ' + elapsed + 'ms (req: ' + script.requestId + ')');
		eventEmitter.emit('done', {requestId: script.requestId, code: code, data: output, start: start, elapsed: elapsed});
	});
	return eventEmitter;
};
...
var isScriptLocal = function (name, version) {
	console.log("IS LOCAL ? " + (versions[name] == version));
...
var addScript = function (name, version, script, callback) {
	...
	var fileContents = new Buffer(script, 'base64').toString('ascii')
