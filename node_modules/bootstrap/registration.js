...
var register = function (service, callback) {
	serversDao.findOne({hostname: os.hostname()}, function (err, server) {
		_registerServer(server, function (server, err) {
			if (server) {
				_registerService(server, service, callback);
			} else {
...
var _registerServer = function (server, callback) {
	...
	if (server) {
		serversDao.update(server._id, serverInfos, function (err, result) {
	...
	// Insert a new server
		...
		serversDao.insert(serverInfos, function (err, result) {
			// If server correctly registered
			if (result !== undefined && result !== null) {
...
var _registerService = function (server, service, callback) {
	// Add service
	if (server.services[service] === undefined) {
		...
		var data = {};
		data['services.' + service] = serviceInfos;
...
/**
 * Unregister the service of the database.
 * If the server of the service is not registered,
 * callback an error... else null
 *
 * @param callback callback an error or null
 */
...
var unregister = function (callback) {
	...
	serversDao.findOne(query, function (err, server) {
		...
		if (server !== undefined && server !== null) {
			if (server.services[service]) {
				...
				serversDao.update(server._id, params, function () {
					logger.info('[Registration] Service unregistered');
					if (callback) callback(null);
				});
			} else {
				if (callback) callback(new Error('[Registration] Service is not registered'));
			}

		}
		// If no server, problem...
		else {
			if (callback) callback(new Error('[Registration] Server is not registered'));
		}
	});
};
