...
var command = function (name, parameters, callback) {
	// Find command in DB
	commands.findOne({name: name}, function (err, dbCommand) {
		...
		if (dbCommand.script !== undefined) {
			// Find script in DB
			scripts.findOne({name: dbCommand.script}, function (err, dbScript) {
				// Execute the script
				var executeScript = function () {
					var scriptToExec = {
						scriptName: 	dbScript.name,
						version: 		dbScript.version,
						arguments: 		prepareArguments(dbCommand.arguments, parameters)
					};
...
var prepareArguments = function (string, parameters) {
	var arguments = new Buffer(string, 'base64').toString('utf-8').split(' ');
	for (var parameter in parameters) {
		var regexp = new RegExp('{{' + parameter + '}}', 'g');
		for (var key in arguments) {
			arguments[key] = arguments[key].replace(regexp, parameters[parameter]);
		}
	}
	return arguments;
};
