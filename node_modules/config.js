 ...
 * =============================
 *
 * Load config for an OMS application from database
 ...
 *
 * Methods :
 *		- load(server, service, [callback])
 *		- get(server, service, [callback])
 *
 * Events : /
 *
 * =============================
...
/**
 * Reload the configuration for the current server
 * and service.
 *
...
var reload = function (callback) {
	if (config.server && config.service) {
		load(config.server, config.service, function (err) {
			if (callback) callback(err);
		});
	} else {
		if (callback) callback(new Error('[Config] Unable to reload if the config is not loaded first'));
	}
};
...
var _loadDefaults = function (conf, callback) {
	...
	configDao.findOne({name: conf.service}, function (err, result) {
...
var _loadService = function (conf, callback) {
	...
	serverDao.findOne(query, function (err, result) {
		// If a config is found
		if (result) {
			// Copy config
...
var _loadServer = function (conf, callback) {
	...
	serverDao.findOne({hostname: conf.server}, function (err, server) {
		// If a config is found
		if (server) {
			// Copy config
