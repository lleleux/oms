...
TLSServer.on('secureConnection', function (stream) {
	// Reject unauthorized connections (bad certificate)
	if (!stream.authorized) {
		logger.info('[Connection] Unauthorized connection from ' + stream.remoteAddress +':'+ stream.remotePort + ' rejected');
		stream.close();
		return;
	}
	// Check if this certificate is already used for a connected agent (double use)
	var agentId = stream.getPeerCertificate().subject.CN;
	if (agents[agentId]) {
		logger.info('[Connection] Double connection détected for certificate/agent ' + agentId + ' rejected');
		stream.close();
		return;
	}
	// Logging
...
var setHandlers = function(stream, agentId) {
	...
	stream.on('close', function(had_error) {
		...
		eventEmitter.emit('disconnection', agentId);
	});
