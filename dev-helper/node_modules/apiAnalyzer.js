...
var tmpPath = config.serviceTmpDir + 'sources/';
// DAO
...
var analyze = function () {
	...
	_getSources(function () {
		_analyzeAPI('api');
		_analyzeAPI('dev-helper');
		logger.info('[API Analyzer] API analyze done for all API\'s');
		_cleanSources();
	});
};
...
var _getSources = function (callback) {
	...
	if (fs.existsSync(tmpPath)) {
		utils.rmdirrfSync(tmpPath);
	}
	fs.mkdirSync(tmpPath);
	// Get git configuration
	configDao.findOne({name: 'git'}, function (err, git) {
		// Create URL
 ...
 */
var _cleanSources = function () {
	utils.rmdirrfSync(tmpPath);
};

/**
 * Analyze the API of the given service.
 *
 * @param service The name of the service source folder to analyze
 */
var _analyzeAPI = function (service) {
	var apiPath = tmpPath + service + '/';
	var rootFile = 'node_modules/server.js';
	logger.info('[API Analyzer] Start API analyse: ' + apiPath);
	// Parsing
	var modules = _parseRootFile(apiPath + rootFile);
	modules.forEach(function (module) {
		_parseModuleComments(apiPath, module)
	});

	// Remove old modules from DB
	var apiDao = new Dao(service, 'doc');
	apiDao._remove({}, {}, function (err, result) {
		// Insert in DB
		modules.forEach(function (module) {
			if (!module.name) {
				module.name = 'Unknown';
			}
			// Get a DAO for the table of the service name
...
var _parseRootFile = function (rootFile) {
	...
	// Search for modules
	var moduleRegExp = new RegExp('\\s*var\\s*' + '(\\S*)Routes' + '\\s=\\srequire\\(' + '(?:\'|")\.\./(routes/\\S*)(?:\'|")\\);');
