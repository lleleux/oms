...
var command = function (hostname, name, parameters, callback) {
	_getHost(hostname, function (host) {
...
var _command = function (isDaemon, hostname, host, name, parameters, callback) {
	...
	commands.findOne({name: name}, function (dbCommand) {
		...
		if (dbCommand.script !== undefined) {
			...
			var filename = config.serverTmpDir + 'scripts/' + dbCommand.script + '_' + dbCommand.version;
			...
			if (host) {
				dns.lookup(host, function (err, ip) {
					if (ip) {
						...
						throw new Error('[MaintenanceCommander] Config hostname not resolvable: ' + host);
					}
				});
			}
			// Hostname
				...
				dns.lookup(hostname, function (err, ip) {
					if (ip) {
						if (callback) callback(hostname);
					} else {
						throw new Error('[MaintenanceCommander] Config hostname not resolvable: ' + hostname);
					}
				});
			}
		} else {
			throw new Error('[MaintenanceCommander] Server not found: ' + hostname);
		}
	});
};
